...
def add_step_list(doc, steps):
    """添加步骤列表 - 使用手动编号确保每章从1开始"""
    for i, step in enumerate(steps, 1):
        para = doc.add_paragraph()
        # 使用手动编号而不是样式，确保每个章节的步骤从1开始
...
def create_manual():
    ...
    rows = [
        ...
        ['聚类分析模块', '基于 Leiden 算法的细胞聚类，自动识别细胞亚群'],
    ...
    add_paragraph_zh(doc, '后端（cluster-End）：基于 FastAPI (Python) 构建，提供文件服务、UMAP 降维算法、Leiden 聚类算法等核心计算服务。')
    ...
    # 插入文件列表面面截图
    add_image_with_caption(doc, os.path.join(pic_dir, '01_file_list_v2.png'),
...
3,11,120.8,180.5,60.3,Sample2'''
    ...
    # 插入数据分析页面截图
    add_image_with_caption(doc, os.path.join(pic_dir, '02_scatter_analysis_v2.png'),
    ...
    # 插入初始配置截图
    add_image_with_caption(doc, os.path.join(pic_dir, '03_initial_config_v2.png'),
    ...
    # 插入维度选择截图
    add_image_with_caption(doc, os.path.join(pic_dir, '04_dimension_select_v2.png'),
    ...
    # 插入生成的散点图截图
    add_image_with_caption(doc, os.path.join(pic_dir, '05_scatter_generated_v2.png'),
    ...
    # 插入聚类参数对话框截图
    add_image_with_caption(doc, os.path.join(pic_dir, '06_cluster_dialog_v2.png'),
    ...
    # 插入热图聚类树截图
    add_image_with_caption(doc, os.path.join(pic_dir, '07_heatmap_cluster_v2.png'),
    ...
    # 插入聚类表截图
    add_image_with_caption(doc, os.path.join(pic_dir, '08_cluster_table_v2.png'),
    ...
    # 插入分析结果页面截图
    add_image_with_caption(doc, os.path.join(pic_dir, '09_results_page_v2.png'),
    ...
    # 保存文档
    output_path = '/Users/yangxiaoyu/学习/all/all-projectR/doc/单细胞数据分析系统-用户功能手册（完善版）.docx'
